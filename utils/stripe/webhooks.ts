...
export async function handleSubscriptionDeleted(event: Stripe.Event) {
  ...
  console.log(
    await supabase.from("unipile_id").select().eq("customer_id", customerId)
  );
...
export async function handleCheckoutCompleted(event: Stripe.Event) {
  ...
  if (!userId) {
    return [{ error: "Missing user_id in metadata" }, { status: 400 }];
  }
...
export async function handleSubscriptionUpdated(event: Stripe.Event) {
  ...
  const customerId = subscription.customer as string;
  if (!customerId) {
    return [{ error: "Missing customerId" }, { status: 400 }];
  }
  ...
  const { data: data_select_credits } = await supabase
    .from("unipile_id")
