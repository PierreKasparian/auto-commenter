        ...
        <div className="w-full max-w-md">
          <Card className="border-none shadow-lg">
            <CardHeader className="space-y-1">
              <CardTitle className="text-3xl font-bold text-center text-gray-900">Welcome to CommentPro</CardTitle>
              <CardDescription className="text-center text-gray-600">Create your account to start automating your LinkedIn interactions</CardDescription>
            </CardHeader>
            <CardContent>
              <Tabs defaultValue="login" className="w-full" value={activeTab} onValueChange={setActiveTab}>
                <TabsList className="grid w-full grid-cols-2 mb-6 gap-2">
                  <TabsTrigger value="login" className="bg-white hover:bg-gray-50">Log In</TabsTrigger>
                  <TabsTrigger value="signup" className="bg-white hover:bg-gray-50">Sign Up</TabsTrigger>
                </TabsList>
                <TabsContent value="login">
                  <form className="space-y-6">
                    ...
                    {isSubmitted ? (
                      <Button className="w-full bg-teal-600 hover:bg-teal-700 text-white py-3" disabled>
                        Logging in...
                      </Button>
                      ...
                      <Button className="w-full bg-teal-600 hover:bg-teal-700 text-white py-3" onClick={handleLogin}>
                        Log in <ArrowRight className="ml-2 h-4 w-4" />
                      </Button>
                    )}
                ...
                <TabsContent value="signup">
                  <form className="space-y-6">
                    ...
                    {isSubmitted ? (
                      ...
                      <Button className="w-full bg-teal-600 hover:bg-teal-700 text-white py-3" onClick={() => handleSignup(email, password, timezone)}>
                        Create account <ArrowRight className="ml-2 h-4 w-4" />
                      </Button>
                    )}
                    <div className="text-center text-sm text-gray-500">
                      <p className="mt-2">By signing up, you agree to our <Link href="/terms" className="text-teal-600 hover:text-teal-700">Terms of Service</Link> and <Link href="/privacy" className="text-teal-600 hover:text-teal-700">Privacy Policy</Link></p>
                    </div>
                  </form>
                </TabsContent>
              </Tabs>
